    ...
    def analyse(self,):
        ...
        with torch.no_grad():
            for batch_id in range(test_num):
                ...
                tp,fp,fn,acc = calculate_metrics_with_postprocess(y_true=y_128,
                                y_pred=y_preds[1])
...
'''
    ...
    def predict(self,saved_folder="../results/LandSliding_LandSlide4Sense/test_imgs"):
        """
        ...
        n_test = self.generator.n_test
        key = 0
        with tqdm(total=self.generator.n_test, desc=f"Prediciting", unit="Image") as pbar:
            for idx in self.generator.test_idxs:
                img,label = self.generator.read_data(idx)
                ...
                y_pred = torch.argmax(y_preds[1],dim=1).squeeze(dim=0).cpu().numpy().astype(np.uint8)
                ...
                # cv2.waitKey(10)
                mask_name = self.generator.data[idx][1].split('/')[-1].replace('h5','png')
                mask_path = os.path.join(saved_folder,mask_name)
                # print(mask_path)
                cv2.imwrite(mask_path,img_draw)
                cv2.waitKey(10)
                pbar.update(1)
                key+=1
