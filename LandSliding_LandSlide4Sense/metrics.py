...
def calculate_metrics_classifier(y_true,y_pred):
    ...
    return tp,fp,fn,acc
def calculate_metrics(y_true:torch.tensor,y_pred:torch.tensor,y_pred_class:torch.tensor,post_process=False,contour_threshold=10):
    """
    ...
    ### Arguments:
        ...
        y_pred_class(Bx1):Class prediction
        post_process: Use opencv to suppress false positive landslide region that has perimeter < contour_threshold
    ### Returns:
        tp,fp,fn,acc
    """
...
def calculate_metrics_with_postprocess(y_true:torch.tensor,y_pred:torch.tensor,post_process=False,contour_threshold=10):
    """
    calculate_metrics(y_true,y_pred): Calculate metrics 
    tp,fp,fn,acc
    ### Arguments:
        y_true (torch.tensor): (BxWxH): ground truth
        y_pred(torch.tensor): (BxCxWxH):Prediction
        post_process: Use opencv to suppress false positive landslide region that has perimeter < contour_threshold
    ### Returns:
        tp,fp,fn,acc
    """
    ...
    for (y_p,y_t) in zip(y_pred,y_true):
        y_p = y_p.cpu().numpy()
        y_t = y_t.cpu().numpy() 
        # print('y_p.shape',y_p.shape,'y_t.shape',y_t.shape)
        y_p = y_p.astype(np.uint8)
        y_t = y_t.astype(np.uint8)
        y_p = remove_small_region_by_contour(y_p)
